...
module ``17: Welcome to the functional fold`` =
    ...
    let ``02 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec Mult lst product = 
                match lst with
                | [] -> product
                | _::rest -> Mult rest (lst.Head * product)
            Mult xs initialState // write a function to multiply the elements of a list
