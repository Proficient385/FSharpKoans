...
module ``19: Other list functions`` =
    ...
    let ``02 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec innerF xs (out1,out2) =
             match xs with
             | [] -> (List.rev out1,List.rev out2)
             | _::rest ->
               match f (xs.Head) with
               | true -> innerF rest ((xs.Head::out1),(out2))
               | _ -> innerF rest ((out1),(xs.Head::out2)) 
            innerF xs ([],[])
    ...
    let ``03 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec innerF i out =
              match i<n with
              | true -> innerF (i+1) (f (i)::out)
              | _ -> List.rev out
            innerF 0 []
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
             let rec innerF xs out =
              ...
              | _::rest -> 
                ...
                | _ -> innerF rest (((p (xs.Head)).Value)::out)
             innerF xs []
    ...
    let ``07 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec innerF xs out i=
              match xs with
              | [] -> List.rev out
                     // f (List.findIndex (xs.Head=xs.Head) xs)
              | _::rest -> innerF rest ((f (i) (xs.Head))::out) (i+1)
